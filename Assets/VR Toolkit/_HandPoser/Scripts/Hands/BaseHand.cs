...
public abstract class BaseHand : MonoBehaviour
{
    ...
    public void ApplyPoseWithPos(Pose pose,out Vector3 pos,out Quaternion rot)
    {
        // Get the proper info using hand's type
        HandInfo handInfo = pose.GetHandInfo(handType);
        ...
        pos = handInfo.attachPosition;
        rot = handInfo.attachRotation;
    }
    ...
    public void ApplyLerpingPose(Pose From, Pose pose, float driveValue)
    {
        // Get the proper info using hand's type
        HandInfo handInfo = pose.GetHandInfo(handType);

        HandInfo handInfoFrom = From.GetHandInfo(handType);

        // Apply rotations 
        ApplyFingerRotationsWithLerp(handInfoFrom.fingerRotations, handInfo.fingerRotations, driveValue);

        // Position, and rotate, this differs on the type of hand
        //ApplyOffset(handInfo.attachPosition, handInfo.attachRotation);
    }
    public void ApplyFingerRotationsWithLerp(List<Quaternion> fromRotations, List<Quaternion> rotations, float driveValue)
    {
        // Make sure we have the rotations for all the joints
        if (HasProperCount(rotations))
        {
            // Set the local rotation of each joint
            for (int i = 0; i < Joints.Count; i++)
                Joints[i].localRotation = Quaternion.Lerp(fromRotations[i], rotations[i], driveValue);
        }
    }
